...
class ChronosHome extends ChronosComponent {
    connectedCallback() {
        this.render(`
            <chronos-hero></chronos-hero>
            <chronos-brand-statement></chronos-brand-statement>
            <chronos-product-showcase></chronos-product-showcase>
            <chronos-features></chronos-features>
            <chronos-gallery></chronos-gallery>
            <chronos-testimonials></chronos-testimonials>
            <chronos-newsletter></chronos-newsletter>
        `);
    }
}
...
class ChronosHeader extends ChronosComponent {
    get navItems() {
        return [
            { label: 'Início', target: 'chronos-home' },
            { label: 'Coleções', target: 'chronos-collections' },
            { label: 'Atelier', target: 'chronos-atelier' },
            { label: 'Herança', target: 'chronos-heritage' }
        ];
    }
        ...
        <header class="sticky top-0 z-50 w-full backdrop-blur-md bg-background-light/80 dark:bg-background-dark/80 border-b border-[#e5e2dc] dark:border-[#2a2a2a]">
            <div class="layout-container flex justify-center w-full">
                <div class="flex items-center justify-between w-full max-w-[1280px] px-6 py-4">
                    ...
                    <div class="flex items-center gap-2 cursor-pointer nav-link" data-target="chronos-home">
                    ...
                    <nav class="hidden md:flex items-center gap-10">
                        ${this.navItems.map(item => `
                            <a class="${this.styles.link}" data-target="${item.target}">
                                ${item.label}
                            </a>
                        `).join('')}
                    </nav>
    ...
    initNavigation() {
        ...
        links.forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                if (contentArea && targetTag) {
                    // Adiciona um efeito de fade simples antes de trocar
                    contentArea.style.opacity = 0;
                    
                    setTimeout(() => {
                        contentArea.innerHTML = `<${targetTag}></${targetTag}>`;
...
class ChronosCollections extends ChronosComponent {
        ...
        return [
            {
                ...
                description: "Onde o ônix encontra a precisão absoluta. Um tributo à escuridão elegante e ao movimento perpétuo.",
                details: "Movimento automático calibre CH-01, reserva de marcha de 72h, caixa de 42mm em cerâmica negra.",
                price: "R$ 12.500"
            },
            {
                id: 2,
                ...
                description: "Ouro 18k e couro italiano trabalhado à mão. A essência da tradição relojoeira suíça.",
                details: "Acabamento em ouro 18k escovado, pulseira em couro de jacaré genuíno, vidro de safira duplo.",
                price: "R$ 18.900"
            },
            {
                id: 3,
                ...
                description: "Mostrador em azul safira profundo e aço cirúrgico. Inspirado na imensidão do céu noturno.",
                details: "Resistência à água 10ATM, mostrador guilloché azul, ponteiros luminescentes em ródio.",
                price: "R$ 9.200"
            },
            {
                id: 4,
                ...
                description: "Leveza extrema com resistência de nível aeroespacial. Para quem desafia os limites do tempo.",
                details: "Caixa em Titânio Grau 5, pulseira de borracha fluorada, bezel giratório unidirecional.",
                price: "R$ 15.400"
            }
        ];
    }
    ...
    connectedCallback() {
        this.render(`
        <section class="w-full py-20 bg-background-light dark:bg-background-dark min-h-screen">
            <div class="max-w-[1280px] mx-auto px-6">
        ...
        <div id="product-modal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 opacity-0 pointer-events-none transition-all duration-500 backdrop-blur-sm bg-black/60">
            <div id="modal-content" class="bg-white dark:bg-surface-dark max-w-4xl w-full max-h-[90vh] overflow-y-auto relative rounded-sm shadow-2xl transform scale-95 transition-transform duration-500">
                </div>
        </div>
        `);
        ...
        <div class="group">
            <div class="relative aspect-[3/4] overflow-hidden bg-slate-100 dark:bg-surface-dark mb-6">
                ...
                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <button class="view-details bg-white text-black px-6 py-3 text-xs font-bold uppercase tracking-widest hover:bg-[#D4AF37] hover:text-white transition-colors" data-id="${item.id}">
                        Ver Detalhes
                    </button>
                </div>
            </div>
            <div class="space-y-2">
                ...
                <h3 class="text-lg font-medium text-slate-900 dark:text-white">${item.name}</h3>
                <span class="text-sm font-light text-slate-500 dark:text-gray-400 block">${item.price}</span>
            </div>
        </div>`;
    }
    ...
    initModalListeners() {
        ...
        const buttons = this.querySelectorAll('.view-details');

        buttons.forEach(btn => {
            btn.addEventListener('click', () => {
                const productId = btn.getAttribute('data-id');
                const product = this.collections.find(p => p.id == productId);
                this.openModal(product, modal, modalContent);
            });
        });
    ...
    openModal(product, modal, container) {
        container.innerHTML = `
            <button id="close-modal" class="absolute top-6 right-6 text-slate-400 hover:text-[#D4AF37] transition-colors z-10">
                <span class="material-symbols-outlined text-3xl">close</span>
            </button>
            <div class="grid grid-cols-1 md:grid-cols-2">
                <div class="h-[400px] md:h-full overflow-hidden">
                    <img src="${product.image}" class="w-full h-full object-cover" />
                </div>
                <div class="p-10 flex flex-col justify-center gap-6">
                    <div>
                        <span class="text-[#D4AF37] uppercase tracking-[0.3em] text-xs font-bold">${product.series}</span>
                        <h2 class="text-3xl font-serif text-slate-900 dark:text-white mt-2">${product.name}</h2>
                        <p class="text-2xl text-slate-400 mt-2 font-light">${product.price}</p>
                    </div>
                    <p class="text-slate-600 dark:text-gray-400 leading-relaxed">${product.description}</p>
                    <div class="border-t border-slate-100 dark:border-white/10 pt-6">
                        <h4 class="text-xs uppercase font-bold text-slate-900 dark:text-white mb-2 tracking-widest">Especificações</h4>
                        <p class="text-sm text-slate-500 dark:text-gray-500 italic">${product.details}</p>
                    </div>
                    <button class="w-full bg-black dark:bg-white text-white dark:text-black py-4 uppercase text-xs font-bold tracking-[0.2em] hover:bg-[#D4AF37] dark:hover:bg-[#D4AF37] hover:text-white transition-all">
                        Reservar Peça
                    </button>
                </div>
            </div>
        ...
        this.querySelector('#close-modal').onclick = () => this.closeModal(modal);
    }
...
customElements.define('chronos-home', ChronosHome);